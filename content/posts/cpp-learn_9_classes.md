...
**A ref-qualifier** allows overloading a member function based on whether it is being called on an `lvalue` or an `rvalue` implicit object.
Using this feature, we can create two versions of getName() -- one for the case where our implicit object is an lvalue, and one for the case where our implicit object is an rvalue.
```cpp
class Employee {
private:
    std::string name_{};
