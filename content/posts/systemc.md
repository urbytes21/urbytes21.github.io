...
- encapsulates `structure (sub-modules)`, `communication (ports/signals)`, and `behavior (processes)`.
- inherits the `sc_module` class.
- used to represent a component in read systems.
  ```cpp
  SC_MODULE(Counter) {
      sc_in<bool>       clk;
      sc_in<bool>       reset;
...
### 6.2. Thread Processes `<SC_THREAD>`
...
### 6.3. Clock Thread Processes `<SC_CTHREAD>`
...
### 6.4. Dynamic Processes `<sc_spawn>`
...
### 10.4. Arbitrary Precision Integers
...
---
## 11. Communication - Interfaces, Ports & Channels
...
### 11.1. Interfaces
**An Interface** (Interface Method Call) defines a set of functions, derived from `sc_interface`, which contains a set of pure virtual functions that shall be defined in one more channels derived from that interface.
...
- `sc_signal` is used to model the behavior of a single piece of wire carrying a digital electronic signal, which can only be written by one process at each delta cycle.
- `sc_signal` implements the `sc_signal_inout_if<T>` interface:
...
`sc_in<T>`,`sc_out<T>` are the specialized port class for use with signals.
...
---
## 18. Trace File & Error/Message Report
