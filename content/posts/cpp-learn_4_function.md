...
- Function delete using `delete` keyword.
- Default-arguments is a default value provided for a function parameter. Parameters with default arguments must always be the rightmost parameters, and they are not used to differentiate functions when resolving overloaded functions.
    ...
    void print_int_or_default(int x, int y = 2) {}

    template <typename T>
    void print_int(T) = delete;
    ...
    void main()
    {
        ...
        print_int_or_default(5);     // prints: 5, 2
        print_int_or_default(5, 10); // prints: 5, 10
    }
    ```
...
---
## 3. Lambda (C++11)
**Lambda** is a convenient way of defining an anonymous function object right at the location where it's invoked or passed as an argument to a function.
**Syntax:**
```cpp
[=] () mutable throw() -> int {
    int n = x + y;
